...
point project_point(matrix H, point p)
{
    ...
    matrix projected_pt = matrix_mult_matrix(H, c);

    q.x = projected_pt.data[0][0] / projected_pt.data[2][0];
    q.y = projected_pt.data[1][0] / projected_pt.data[2][0];
    return q;
}
...
float point_distance(point p, point q)
{
    ...
    float l2_distance = sqrtf((p.x - q.x) * (p.x - q.x) + (p.y - q.y) * (p.y - q.y));
...
int model_inliers(matrix H, match *m, int n, float thresh)
{
    ...
    for (int i = 0; i < n; i++)
    {
        point q_prime = project_point(H, m[count].p);
        if (point_distance(q_prime, m[count].q) < thresh)
        {
            count++;
        }
        else
...
image combine_images(image a, image b, matrix H)
{
    ...
    for (k = 0; k < a.c; ++k)
    {
        for (j = topleft.y; j < botright.y; ++j)
        {
            ...
            {
                point projected_to_b = project_point(H, make_point(i, j));
                if (projected_to_b.x >= 0 && projected_to_b.y >= 0 && projected_to_b.x < b.w && projected_to_b.y < b.h)
                {
                    float pixel_value = bilinear_interpolate(b, projected_to_b.x, projected_to_b.y, k);
    ...
    for (int k = 0; k < c.c; ++k)
    {
        for (int y = 0; y < c.h; ++y)
        {
            for (int x = -w / 2; x <= w / 2; ++x)
            {
                ...
                float pixel_value = bilinear_interpolate(im, x_prime, y_prime, k);
                set_pixel(c, x + w / 2, y, k, pixel_value);
            }
        }
    }
    return c;
