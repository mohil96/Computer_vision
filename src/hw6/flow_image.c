    ...
    for (int c = 0; c < im.c; c++)
    {
        for (int y = 0; y < im.h; y++)
        {
            for (int x = 0; x < im.w; x++)
            {
                ...
                {
                    sum = get_pixel(im, x, y, c) + get_pixel(integ, x, y - 1, c);
                }
                else if ((y - 1) < 0)
                {
                    sum = get_pixel(im, x, y, c) + get_pixel(integ, x - 1, y, c);
                }
                else
                {
                    sum = get_pixel(im, x, y, c) + get_pixel(integ, x - 1, y, c) + get_pixel(integ, x, y - 1, c) - get_pixel(integ, x - 1, y - 1, c);
                }
...
image box_filter_image(image im, int s)
{
    ...
    for (k = 0; k < im.c; k++)
    {
        for (j = 0; j < im.h; j++)
        {
            for (i = 0; i < im.w; i++)
            {
                float pixel_value = get_pixel(integ, i + offset, j + offset, k) - get_pixel(integ, i + offset, j - offset - 1, k) - get_pixel(integ, i - offset - 1, j + offset, k) + get_pixel(integ, i - offset - 1, j - offset - 1, k);
                pixel_value /= powf(s, 2);
...
image time_structure_matrix(image im, image prev, int s)
{
    ...
    for (j = 0; j < im.h; j++)
    {
        for (i = 0; i < im.w; i++)
        {
            ...
            set_pixel(S, i, j, 0, powf(ix, 2));
            set_pixel(S, i, j, 1, powf(iy, 2));
    ...
    return convolve_image(S, make_box_filter(s), 1);
}
